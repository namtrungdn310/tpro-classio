    ...
    async with AsyncSessionLocal() as db:
        ...
        for page in range(1, 8):
            result = await read_schedule_fees(
                db, enrollment.id, year=0, include_inactive=True, order="asc", page=page
            )
        ...
        assert (await read_schedule_fees(db, enrollment.id, year=2016))["total"] == 10
        assert (await read_schedule_fees(db, enrollment.id, year=2016, state="PAID"))[
            "total"
        ] == 1
        ...
        recent = await read_schedule_fees(db, enrollment.id, year=2025, state="PAID")
